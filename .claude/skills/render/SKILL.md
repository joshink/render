...
- **Per-frame noise balloons MP4 size**: `film_grain` (and any effect that re-randomizes every frame) defeats H.264 inter-frame prediction — a 1080p render can jump from tens of MB to gigabytes and encode ~10× slower. When authoring such a spec with a direct `.mp4` output, cap it up front via the output's `encode` block: `"output": { "path": "out.mp4", "encode": { "crf": 28, "max_bitrate": "12M" } }` (KDL: `output "out.mp4" { encode crf=28 max_bitrate="12M" }`). For `mux://` output the size only costs upload time — Mux re-encodes for delivery. The engine logs size/bitrate after every encode and warns when the bitrate is anomalously high.
